...
fun main(args: Array<String>) = Config(WorkbookFactory.create(FileInputStream(args[0])).getSheetAt(0)
    .drop(1).map { r -> r.take(6).map { "$it" } }.groupBy { it[0] }
    .mapValues { (_,v) -> v.associate { it[1] to Card(it[2], it[3], it[4].toBoolean(), it.getOrElse(5) { "" }) } }
).let(Yaml()::encodeToString).replace(""""(.+)":""".toRegex()) { "${it.groupValues[1]}:" }.let(File("output.yml")::writeText)